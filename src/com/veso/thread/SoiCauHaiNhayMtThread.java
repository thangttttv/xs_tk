...
public class SoiCauHaiNhayMtThread {
	public final static long miliSecondDay = 86400000;
	public final static String danang ="dng";
	private static Logger logger = new Logger("SoiCauHaiNhayMtThread");
	public boolean createBuocCauMienTrung(String boso,int x, int y, long bien_ngay, int buoc,int loaicau, int songay,BuocCauBean buocCauBean,int province_id,String province_code)
	{
		...
		if(lotoDAO.checkLoto2NhayMienTrung(open_date, boso,province_id))
		{
			if(songay==buoc)
			{
	...
	public static void main(String[] args) {
		...
		logger.log("Soi cau Hai nhay Mien Trung ngay "+Calendar.getInstance().get(Calendar.DAY_OF_MONTH)+"/"+(Calendar.getInstance().get(Calendar.MONTH)+1)+"/"+Calendar.getInstance().get(Calendar.YEAR));
		
		KetQuaMienTrungDAO ketQuaMienTrungDAO = new KetQuaMienTrungDAO();
		SoiCauDAO soiCauDAO = new SoiCauDAO();
		BuocCauDAO buocCauDAO = new BuocCauDAO();		
		// Tao cau
		SoiCauHaiNhayMtThread soiCauHaiNhayMtThread = new SoiCauHaiNhayMtThread();
		...
		// xac dinh bien ngay tren neu la dn
							...
							while(i<daySoBean.getDayso().length)
							{
								...
								for(j=i+1;j<daySoBean.getDayso().length;j++)
								{
									if(daySoBean.getDayso()[j]!=null)
									{
										...
										kq=soiCauHaiNhayMtThread.createBuocCauMienTrung(boso, i, j, bien_ngay_duoi, 2, locai_cau[q], so_ngay[k], buocCauBean,provinceBean.getId(),provinceBean.getCode());
										if(kq)
										{
											logger.log("---------------->Tao cau boso "+boso+" vi tri xy ="+i+""+j);
											buocCauBean.setKieu_cau(locai_cau[q]);
											buocCauBean.setBoso(Integer.parseInt(soiCauHaiNhayMtThread.getBoSo(buocCauBean, so_ngay[k])));
											buocCauDAO.save(buocCauBean);
											soiCauDAO.updateBoso(cau_id, "bs_"+soiCauHaiNhayMtThread.getBoSo(buocCauBean, so_ngay[k]));
										}
										}catch (Exception e) {
											e.printStackTrace();
										}
