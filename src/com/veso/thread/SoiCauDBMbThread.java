...
public class SoiCauDBMbThread {
	private static Logger logger = new Logger("SoiCauDBMbThread");
	...
	public boolean createBuocCauMienBac(String boso,int x, int y, long bien_ngay, int buoc,int loaicau, int songay,BuocCauBean buocCauBean)
	{
		...
		if(lotoDAO.checkDeMienBac(open_date, boso))
		{
			if(songay==buoc)
			{
	...
	public static void main(String[] args) {
		logger.log("Soi cau Dac biet Mien Bac ngay "+Calendar.getInstance().get(Calendar.DAY_OF_MONTH)+"/"+(Calendar.getInstance().get(Calendar.MONTH)+1)+"/"+Calendar.getInstance().get(Calendar.YEAR));
		KetQuaMienBacDAO ketQuaMienBacDAO = new KetQuaMienBacDAO();
		SoiCauDAO soiCauDAO = new SoiCauDAO();
		BuocCauDAO buocCauDAO = new BuocCauDAO();		
		// Tao cau
		SoiCauDBMbThread sMienBacThread = new SoiCauDBMbThread();
