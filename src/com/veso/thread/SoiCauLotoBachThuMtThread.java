...
public class SoiCauLotoBachThuMtThread {
	
	public final static long miliSecondDay = 86400000;	
	public final static String danang ="dng";
	private static Logger logger = new Logger("SoiCauLotoMtThread");
	...
	public boolean createBuocCauMienTrung(String boso,int x, int y, long bien_ngay, int buoc,int loaicau, int songay,BuocCauBean buocCauBean,int province_id,String province_code)
	{
		...
		if(lotoDAO.checkLotoMienTrungBachThu(open_date, boso,province_id)>0)
		{
			if(songay==buoc)
			{
				daySoBean = ketQuaMienTrungDAO.getDaySoMT(open_date,province_id);
				if(x>=daySoBean.getDayso().length||y>=daySoBean.getDayso().length) return false;
				if(daySoBean!=null)
				{
			...
			if(buoc<songay)
			{
				...
				{
					dayso = daySoBean.getDayso();
					/*logger.log("---------------->createBuocCauMienTrung Tinh"+province_id+" ngay "+open_date.toString()+" vi tri xy ="+x+"-"+y);
					logger.log("---------------->createBuocCauMienTrung Lengt day so"+daySoBean.getDayso().length);*/
					boso = String.valueOf(dayso[x]+""+dayso[y]);
					setBuocCau(buocCauBean,buoc,boso,daySoBean.getKequa_id());
					buoc +=1; 
	...
	public static void main(String[] args) {
		logger.log("Soi cau loto Mien Trung ngay "+Calendar.getInstance().get(Calendar.DAY_OF_MONTH)+"/"+(Calendar.getInstance().get(Calendar.MONTH)+1)+"/"+Calendar.getInstance().get(Calendar.YEAR));
		KetQuaMienTrungDAO ketQuaMienTrungDAO = new KetQuaMienTrungDAO();
		SoiCauDAO soiCauDAO = new SoiCauDAO();
		BuocCauDAO buocCauDAO = new BuocCauDAO();		
		// Tao cau
		SoiCauLotoBachThuMtThread soiCauLotoMtThread = new SoiCauLotoBachThuMtThread();
		...
		for (ProvinceBean provinceBean : prList) {
			//if(provinceBean.getId()!=36) continue;
							...
							while(i<daySoBean.getDayso().length)
							{
								...
								for(j=i+1;j<daySoBean.getDayso().length;j++)
								{
									if(daySoBean.getDayso()[j]!=null)
									{
										...
										kq=soiCauLotoMtThread.createBuocCauMienTrung(boso, i, j, bien_ngay_duoi, 2, locai_cau[q], so_ngay[k], buocCauBean,provinceBean.getId(),provinceBean.getCode());
										if(kq)
										{
											logger.log("---------------->Tao cau boso "+provinceBean.getId()+" "+boso+" vi tri xy ="+i+"-"+j);
											buocCauBean.setKieu_cau(locai_cau[q]);
											buocCauBean.setBoso(Integer.parseInt(soiCauLotoMtThread.getBoSo(buocCauBean, so_ngay[k])));
											buocCauDAO.save(buocCauBean);
											soiCauDAO.updateBoso(cau_id, "bs_"+soiCauLotoMtThread.getBoSo(buocCauBean, so_ngay[k]));
										}
										}catch (Exception e) {
											e.printStackTrace();
										}
