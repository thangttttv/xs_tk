...
public class SoiCauHaiNhayMbThread {
	
	public final static long miliSecondDay = 86400000;
	private static Logger logger = new Logger("SoiCauHaiNhayMbThread");
	...
	public boolean createBuocCauMienBac(String boso,int x, int y, long bien_ngay, int buoc,int loaicau, int songay,BuocCauBean buocCauBean)
	{
		...
		if(lotoDAO.checkLoto2NhayMienBac(open_date, boso))
		{
			if(songay==buoc)
			{
	...
	public static void main(String[] args) {
		logger.log("Soi cau Hai nhay Mien Bac ngay "+Calendar.getInstance().get(Calendar.DAY_OF_MONTH)+"/"+(Calendar.getInstance().get(Calendar.MONTH)+1)+"/"+Calendar.getInstance().get(Calendar.YEAR));
		
		KetQuaMienBacDAO ketQuaMienBacDAO = new KetQuaMienBacDAO();
		SoiCauDAO soiCauDAO = new SoiCauDAO();
		BuocCauDAO buocCauDAO = new BuocCauDAO();		
		// Tao cau
		SoiCauHaiNhayMbThread sMienBacThread = new SoiCauHaiNhayMbThread();
						...
						for(j=i+1;j<daySoBean.getDayso().length;j++)
						{
							if(daySoBean.getDayso()[j]!=null)
							{
								try
								{
									...
									kq=sMienBacThread.createBuocCauMienBac(boso, i, j, bien_ngay_duoi, 2, locai_cau[q], so_ngay[k], buocCauBean);
									if(kq)
									{	logger.log("------------>Tao cau boso " + boso+" Vi chi xy = "+i+""+"j");
										buocCauBean.setKieu_cau(locai_cau[q]);
										buocCauBean.setBoso(Integer.parseInt(sMienBacThread.getBoSo(buocCauBean, so_ngay[k])));
										buocCauDAO.save(buocCauBean);
										soiCauDAO.updateBoso(cau_id, "bs_"+sMienBacThread.getBoSo(buocCauBean, so_ngay[k]));
									}
								}catch (Exception e) {
									e.printStackTrace();
								}
