...
public class TransactionOutDAO {
	...
	public boolean save(TransactionIn transactionIn) {
		...
		try {
			...
			strSQL = new StringBuffer("INSERT INTO transaction_in (user_id,ticket_id, tk_chinh, content, transaction_date)" +
					" VALUES  (?,?,?,?,?) ");
	...
	public List<TransactionIn> getAllTransaction() {
		...
		try {
			...
			strSQL = new StringBuffer(
					"SELECT 	id, user_id, user_name FROM az24_veso.transaction_out order by id ");
			preStmt = conn.prepareStatement(strSQL.toString());		
	...
	public boolean UpdateUserName(TransactionIn transactionIn) {
		...
		try {
			conn = C3p0VesoPool.getConnection();		
			conn.setAutoCommit(false);
			strSQL = new StringBuffer("Update  transaction_out Set user_name = ? Where id = ? ");
			preStmt = conn.prepareStatement(strSQL.toString());
	...
	public static void main(String[] args) {
		TransactionOutDAO transactionInDAO = new TransactionOutDAO();
