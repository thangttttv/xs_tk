...
public class NoticeIOSDAO {
	...
	public List<Notice> getNoticeToSendQueue() {
		...
		try {
			...
			strSQL = new StringBuffer(
					"SELECT id,title,image,url,description,TYPE,STATUS,create_date,create_user,time_sent FROM vtc_10h_xs.xs_notice_ios WHERE STATUS = 1");
	...
	public boolean updateStatus(int notice_id,int status) {
		...
		try {
			...
			strSQL = new StringBuffer("UPDATE vtc_10h_xs.xs_notice_ios SET STATUS = 1 WHERE id = ? ");
	...
	public boolean checkNoticeQueueExist(int notice_id,String device_token) {
		Connection conn = null;
		PreparedStatement preStmt = null;
		StringBuffer strSQL = null;
		ResultSet rs = null;
		...
		try {
			conn = C3p0VesoPool.getConnection();			
			strSQL = new StringBuffer(
					"SELECT * FROM vtc_10h_xs.xs_notice_ios_queue WHERE notice_id = ? AND device_token = ?");
			
			preStmt = conn.prepareStatement(strSQL.toString());			
			preStmt.setInt(1, notice_id);
			preStmt.setString(2, device_token);
	...
	public List<NoticeUser> getNoticeUser() {
		...
		try {
			conn = C3p0VesoPool.getConnection();			
			strSQL = new StringBuffer(
					"SELECT id,app_client_id,device_token, username, ip_address, create_date FROM vtc_10h_xs.xs_notice_ios_user ");
	...
	public boolean insertNoticeQueue(NoticeQueue notice) {
		...
		try {
			...
			conn.setAutoCommit(false);
			strSQL = new StringBuffer(
					" INSERT INTO vtc_10h_xs.xs_notice_ios_queue (notice_id, device_token, payload, time_queued,time_sent )" +
					" VALUES ( ?, ?, ?, NOW(),?) ;");

			preStmt = conn.prepareStatement(strSQL.toString());
			preStmt.setInt(1, notice.notice_id);
			preStmt.setString(2,notice.device_token);
	...
	public boolean deleteQueue(int id) {
		...
		try {
			conn = C3p0VesoPool.getConnection();	
			conn.setAutoCommit(false);
			strSQL = new StringBuffer("DELETE FROM vtc_10h_xs.xs_notice_ios_queue  WHERE id = ? ");
			preStmt = conn.prepareStatement(strSQL.toString());
