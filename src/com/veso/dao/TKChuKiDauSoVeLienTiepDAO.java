...
public class TKChuKiDauSoVeLienTiepDAO {
	...
	public boolean save(TKChuKiDauSoVeLienTiep chuKyBoSo) {
		...
		try {
			...
			strSQL = new StringBuffer(" INSERT INTO thongke_dau_ve_lientiep (province_id, dau, start_date, end_date, length" +
					"	) VALUES (	?,	?,	?,	?,	? )");
	...
	public TKChuKiDauSoVeLienTiep getChuKyCuoi(String boso,int province_id) {
		Connection conn = null;
		PreparedStatement preStmt = null;
		StringBuffer strSQL = null;
		ResultSet rs = null;		
		TKChuKiDauSoVeLienTiep dauSoVeLienTiep = null;
		try {

			conn = C3p0VesoPool.getConnection();			
			strSQL = new StringBuffer(
					"   SELECT 	id,province_id,dau,start_date,end_date,length " +
					" 	FROM thongke_dau_ve_lientiep WHERE dau = ? AND province_id = ? " +
	...
	public List<TKChuKiDauSoVeLienTiep> getChuKyCanXoa(int province_id,Date open_date) {
		Connection conn = null;
		PreparedStatement preStmt = null;
		StringBuffer strSQL = null;
		ResultSet rs = null;		
		TKChuKiDauSoVeLienTiep boSoVeLienTiep = null;
		List<TKChuKiDauSoVeLienTiep> chukis = new ArrayList<TKChuKiDauSoVeLienTiep>();
		try {

			conn = C3p0VesoPool.getConnection();			
			strSQL = new StringBuffer(
					"   SELECT 	id,province_id,dau,start_date,end_date,length " +
					" 	FROM thongke_dau_ve_lientiep WHERE  province_id = ? AND  end_date < ? AND length = 1 " +
					"   ORDER BY id DESC ");
	...
	public boolean updateLengthChuKy(TKChuKiDauSoVeLienTiep chuKyBoSo) {
		Connection conn = null;
		PreparedStatement preStmt = null;
		StringBuffer strSQL = null;
		boolean result = false;
		try {
			conn = C3p0VesoPool.getConnection();
			conn.setAutoCommit(false);
			strSQL = new StringBuffer(" UPDATE  thongke_dau_ve_lientiep " +
	...
	public boolean deleteChuKy(int id) {
		...
		try {
			conn = C3p0VesoPool.getConnection();
			conn.setAutoCommit(false);
			strSQL = new StringBuffer(" DELETE FROM  thongke_dau_ve_lientiep WHERE id = ? ");
	...
	public boolean dongChuKi(TKChuKyBoSo chuKyBoSo) {
		...
		try {
			conn = C3p0VesoPool.getConnection();
			conn.setAutoCommit(false);
			strSQL = new StringBuffer(" UPDATE  thongke_dau_ve_lientiep SET end_date = ?  WHERE id = ? ");
	...
	public List<TKChuKiDauSoVeLienTiep> findAll(int start) {
		Connection conn = null;
		PreparedStatement preStmt = null;
		StringBuffer strSQL = null;
		ResultSet rs = null;		
		TKChuKiDauSoVeLienTiep dauSoVeLienTiep = null;
		List<TKChuKiDauSoVeLienTiep> list = new ArrayList<TKChuKiDauSoVeLienTiep>();
		try {

			conn = C3p0VesoPool.getConnection();			
			strSQL = new StringBuffer(
					"   SELECT 	id,province_id,dau,start_date,end_date,length " +
					" 	FROM thongke_dau_ve_lientiep WHERE id > ? " +
					"   ORDER BY id  ");
