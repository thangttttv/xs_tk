...
public class TKChuKiBoSoVeLienTiepDAO {
	private Logger logger = new Logger(this.getClass().getName());

	public boolean save(TKChuKiBoSoVeLienTiep chuKyBoSo) {
		...
		try {
			...
			strSQL = new StringBuffer(" INSERT INTO thongke_boso_ve_lientiep ( province_id,	boso, start_date,end_date," +
					" length) VALUES (	?,	?,	?,	?,	? )");
	...
	public TKChuKiBoSoVeLienTiep getChuKyCuoi(String boso,int province_id) {
		Connection conn = null;
		PreparedStatement preStmt = null;
		StringBuffer strSQL = null;
		ResultSet rs = null;		
		TKChuKiBoSoVeLienTiep boSoVeLienTiep = null;
		try {

			conn = C3p0VesoPool.getConnection();			
			strSQL = new StringBuffer(
					"   SELECT 	id,province_id,boso,start_date,end_date,length " +
					" 	FROM thongke_boso_ve_lientiep WHERE boso = ? AND province_id = ? " +
		...
		return boSoVeLienTiep;
	}
	
	public List<TKChuKiBoSoVeLienTiep> getChuKyCanXoa(int province_id,Date open_date) {
		Connection conn = null;
		PreparedStatement preStmt = null;
		StringBuffer strSQL = null;
		ResultSet rs = null;		
		TKChuKiBoSoVeLienTiep boSoVeLienTiep = null;
		List<TKChuKiBoSoVeLienTiep> chukis = new ArrayList<TKChuKiBoSoVeLienTiep>();
		try {

			conn = C3p0VesoPool.getConnection();			
			strSQL = new StringBuffer(
					"   SELECT 	id,province_id,boso,start_date,end_date,length " +
					" 	FROM thongke_boso_ve_lientiep WHERE  province_id = ? AND  end_date < ? AND length = 1 " +
					"   ORDER BY id DESC ");
	...
	public boolean updateLengthChuKy(TKChuKiBoSoVeLienTiep chuKyBoSo) {
		Connection conn = null;
		PreparedStatement preStmt = null;
		StringBuffer strSQL = null;
		boolean result = false;
		try {
			conn = C3p0VesoPool.getConnection();
			conn.setAutoCommit(false);
			strSQL = new StringBuffer(" UPDATE  thongke_boso_ve_lientiep " +
	...
	public boolean deleteChuKy(int id) {
		...
		try {
			conn = C3p0VesoPool.getConnection();
			conn.setAutoCommit(false);
			strSQL = new StringBuffer(" delete FROM  thongke_boso_ve_lientiep WHERE id = ? ");
	...
	public boolean dongChuKi(TKChuKyBoSo chuKyBoSo) {
		...
		try {
			conn = C3p0VesoPool.getConnection();
			conn.setAutoCommit(false);
			strSQL = new StringBuffer(" UPDATE  thongke_chuky_boso SET end_date = ?  WHERE id = ? ");
	...
	public List<TKChuKiBoSoVeLienTiep> findAll(int start) {
		Connection conn = null;
		PreparedStatement preStmt = null;
		StringBuffer strSQL = null;
		ResultSet rs = null;		
		...
		try {
			...
			strSQL = new StringBuffer(
					"   SELECT 	id,province_id,boso,start_date,end_date,length " +
					" 	FROM thongke_boso_ve_lientiep WHERE  id > ?  " +
					"   ORDER BY id  ");
